...
from .models import  Medicamento, RegistroUso
...
class MedicamentoForm(forms.ModelForm):
    ...
    horarios_texto = forms.CharField(
        ...
        label = "Horarios de uso",
        help_text="Informa cada u dos horários separados por virgulas",
    ...
    def clean_horarios_texto(self):
        ...
        for h in horarios:
            try: 
                ...
                raise forms.ValidationError(f"Horário Invalido, use a forma Horas:Minutos.")
        return horarios
    def clean_nome(self):
        nome = self.cleaned_data.get("nome", "").strip()
        if not nome:
            raise forms.ValidationError("O nome não pode ser nulo.")
        return nome

    def clean_dose(self):
        dose = self.cleaned_data.get("dose", "").strip()
        if not dose:
            raise forms.ValidationError("A dose não pode ser nula.")
        return dose
...
class FiltroForm(forms.Form):
    ...
    importancia = forms.CharField(
        required=False,
        label= "Buscar por importancia",
        choices=[
            ("", "Todos"),
            ("baixo", "Baixo"),
            ("medio", "Médio"),
